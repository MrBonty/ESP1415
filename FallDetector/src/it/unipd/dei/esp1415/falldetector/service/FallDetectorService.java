...
public class FallDetectorService extends Service {
	...
	// Arrays which contain accelerometer data
	private float[] accDataX;
	private float[] accDataY;
	private float[] accDataZ;
	private int accDataIndex;

	// Class which contains a single accelerometer data
	private AccelData newData;

	// Long variables used for calculating the sample rates
	...
	public void onCreate() {
		...
		ArrayList<AccelData> tmpAccData = dm
				.getTempAccelDataAsArray(DatabaseTable.COLUMN_PK_ID + " "
						+ DatabaseManager.ASC);
		...
		if (accDataIndex >= FallAlgorithmUtility.ACC_DATA_SIZE) {
			...
			accDataIndex = accDataIndex + FallAlgorithmUtility.ACC_DATA_SIZE
					+ 1;
		}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent != null) {
			...
			if (intent.getBooleanExtra(FallDetectorService.GET_ARRAY, false)) {
				...
				broadcastDataIntent.putExtra(FallDetectorService.ARRAY_SIZE,
						accDataIndex);
				broadcastDataIntent.putExtra(FallDetectorService.X_AXIS_ARRAY,
						accDataX);
				broadcastDataIntent.putExtra(FallDetectorService.Y_AXIS_ARRAY,
						accDataY);
				broadcastDataIntent.putExtra(FallDetectorService.Z_AXIS_ARRAY,
						accDataZ);
	...
	private final LocationListener locationListener = new LocationListener() {
		...
		public void onLocationChanged(Location location) {
			// Called when a new location is find by the network location
			// provider, not GPS.
			if (location != null) {
				latitude = location.getLatitude();
				longitude = location.getLongitude();
					...
					if (isPlaying) {
						...
						if (accDataIndex >= FallAlgorithmUtility.ACC_DATA_SIZE) {
							newData.setId(accDataIndex
									% FallAlgorithmUtility.ACC_DATA_SIZE);
							newData.setTimestamp(event.timestamp);
							newData.setX((double) (accDataX[accDataIndex
									% FallAlgorithmUtility.ACC_DATA_SIZE] = event.values[0]));
							newData.setY((double) (accDataY[accDataIndex
									% FallAlgorithmUtility.ACC_DATA_SIZE] = event.values[1]));
							newData.setZ((double) (accDataZ[accDataIndex
									% FallAlgorithmUtility.ACC_DATA_SIZE] = event.values[2]));
							lastSampleTime = System.currentTimeMillis();
							dm.upgradeATempAccelData(newData);
							broadcastDataIntent.putExtra(
									FallDetectorService.ARRAY_SIZE,
									accDataIndex);
						...
						broadcastDataIntent.putExtra(
								FallDetectorService.X_AXIS_ARRAY, accDataX);
						broadcastDataIntent.putExtra(
								FallDetectorService.Y_AXIS_ARRAY, accDataY);
						broadcastDataIntent.putExtra(
								FallDetectorService.Z_AXIS_ARRAY, accDataZ);
						broadcastDataIntent.putExtra(
								FallDetectorService.TIME_EXCEED, timeExceed);
						broadcaster.sendBroadcast(broadcastDataIntent);
						accDataIndex++;

						if (timeExceed) {
							ContentValues values = new ContentValues();
							values.put(DatabaseTable.COLUMN_SS_DURATION,
									sessionMaxDuration);
							...
							if (ListSessionFragment.mArray != null) {
								ListSessionFragment.mArray.get(0).setDuration(
										sessionMaxDuration);
								ListSessionFragment.mArray.get(0).setToActive(
										false);
							}
	...
	private void fallDetected(AccelData data) {
		...
		Fall fall = new Fall(
				(myTime + (data.getTimestamp() - sensorTime) / 1000000l), dm
						.getLastSession().getId());

		fall.setLatitude(latitude);
		fall.setLongitude(longitude);
		fall.setId(dm.insertAFall(fall));

		if ((CurrentSessionActivity.arrayAdapter != null)
				&& (CurrentSessionActivity.falls != null)) {
			CurrentSessionActivity.falls.add(fall);
			CurrentSessionActivity.lstvFalls
					.setAdapter(CurrentSessionActivity.arrayAdapter);
	...
	/**
	 * [m] Method to save Accelerator data on Database and share mail
	 */
	private void saveAndShare() {
		...
		int maxDataPos = (FallAlgorithmUtility.GET_HALF_SEC + posTmp)
				% FallAlgorithmUtility.ACC_DATA_SIZE;

		long fallId = fallTmp.getId();
		for (int i = 1; i <= FallAlgorithmUtility.DATA_SAVE; i++) {
			AccelData temp = new AccelData(FallAlgorithmUtility.DATA_SAVE + 1
					- i, fallId);
		...
		if (ListSessionFragment.mArray != null) {
			ListSessionFragment.mArray.get(0).setFallsNum(
					last.getFallsNum() + 1);
		}
		...
		if (status.hasMobileConnectionON() || status.hasWifiConnectionON()) {
			...
			String sbj = getApplicationContext().getResources().getString(
					R.string.mail_sbj);

			String msg = getApplicationContext().getResources().getString(
					R.string.mail_msg1);
			msg = msg + cal.get(Calendar.HOUR_OF_DAY) + ":"
					+ cal.get(Calendar.MINUTE) + " of "
			...
			msg = msg
					+ getApplicationContext().getResources().getString(
							R.string.latitude) + ":" + fallTmp.getLatitude()
					+ "\n";
			msg = msg
					+ getApplicationContext().getResources().getString(
							R.string.longitude) + ":" + fallTmp.getLongitude()
					+ "\n";
			msg = msg
					+ getApplicationContext().getResources().getString(
							R.string.mail_end1) + "\n\n";
			...
			SharedPreferences preferences = PreferenceManager
					.getDefaultSharedPreferences(getApplicationContext());
			if (preferences.getBoolean(SettingsMainFragment.SAVE_MAIL_CHK,
					false)) {
				...
				String[] data = preferences.getString(
						SettingsMainFragment.SAVE_MAIL_DATABASE64, "").split(
						SettingsActivity.DIVISOR_DATA);
				String sender = preferences.getString(
						SettingsMainFragment.SAVE_MAIL_ACCOUNT, "");

				if (data.length > 1) {
					MailSender mailSend = new MailSender(MailSender.GMAIL_PORT,
							MailSender.GMAIL_SERVER, data[0], data[1], false);
				...
				if (sendTo.resolveActivity(getPackageManager()) != null) {
					sendTo.setFlags(sendTo.getFlags()
			...
			if (isSend) {
				...
				if ((CurrentSessionActivity.arrayAdapter != null)
						&& (CurrentSessionActivity.falls != null)) {
					CurrentSessionActivity.falls.get(
							CurrentSessionActivity.falls.size() - 1)
							.setNotification(isSend);
					CurrentSessionActivity.lstvFalls
							.setAdapter(CurrentSessionActivity.arrayAdapter);
				}
			}
