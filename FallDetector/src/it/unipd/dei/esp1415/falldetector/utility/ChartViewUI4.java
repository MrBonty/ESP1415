 ...
 * of data */
public class ChartViewUI4 extends View{
	 ...
	 */
	public ChartViewUI4(Context context, AttributeSet attrs) {
		super(context, attrs);
...
//		}
		if(viewWidht >= 2*size){
			int dim = viewWidht / size;
			for(int i = 0; i < size - 1; i++){
				canvas.drawLine((float) (dim * i), chartData[i] + (viewHeight / 2),
						(float) (dim * (i + 1)), chartData[i + 1] + (viewHeight / 2), paint);
			}
		}
	}
